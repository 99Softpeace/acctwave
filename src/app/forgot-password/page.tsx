...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccess(`If an account exists with this ${identifierType}, you will receive a password reset ${identifierType === 'email' ? 'link' : 'code'}.`);
        } catch (err: any) {
                ...
                <div className="relative rounded-3xl overflow-hidden backdrop-blur-[2px] bg-transparent border border-white/10 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)]">
                    ...
                    <div className="relative z-30 p-8">
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-2xl font-bold text-white mb-2 drop-shadow-md">Forgot Password</h1>
                            <p className="text-gray-200 text-sm font-medium drop-shadow-sm">
                                Enter your {identifierType} to receive a reset {identifierType === 'email' ? 'link' : 'code'}
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="flex bg-white/5 rounded-xl p-1 mb-6 border border-white/10">
                                <button
                                    ...
                                    className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${identifierType === 'email' ? 'bg-primary text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                                >
                                    Email Address
                                </button>
                                <button
                                    ...
                                    className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${identifierType === 'phone' ? 'bg-primary text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}
                                >
                                    Phone Number
                                </button>
                            </div>
                        ...
                        <div className="mt-6 text-center">
                            <Link href="/login" className="text-gray-300 text-sm font-medium hover:text-white transition-colors drop-shadow-sm">
                                Back to Login
                            </Link>
                        </div>
                    </div>
